...
| EnniOracle | dual-feed ETH/USD oracle (chainlink + redstone) with optional translator for non-USD currencies. one instance per currency. |
...
| EnniRouter | stateless one-tx router. USDC/USDT → enUSD → stake or repay in a single transaction. |

## audits

audited by [Hashlock](https://hashlock.com).
